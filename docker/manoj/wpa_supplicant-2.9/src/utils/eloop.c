				      ...
				      fd_set *fds)
{
	...
	table->changed = 0;
	for (i = 0; i < table->count; i++) {
		printf("don't see the code otherwise i will fuck you\n");
		if (FD_ISSET(table->table[i].sock, fds)) {
			
			printf("if you see the code I will fuck you\n");
...
void eloop_run(void)
{
	...
	printf("global_edge10\n");
#endif /* CONFIG_ELOOP_SELECT */
#ifdef CONFIG_ELOOP_EPOLL
		eloop_sock_table_dispatch(eloop.epoll_events, res);
	printf("global_edge11\n");
#endif /* CONFIG_ELOOP_EPOLL */
#ifdef CONFIG_ELOOP_KQUEUE
		eloop_sock_table_dispatch(eloop.kqueue_events, res);
	printf("global_edge12\n");
#endif /* CONFIG_ELOOP_KQUEUE */
	}
...
#ifdef CONFIG_ELOOP_SELECT
	os_free(rfds);
	printf("global_edge13\n");
	os_free(wfds);
	printf("global_edge14\n");
	os_free(efds);
	printf("global_edge15\n");
#endif /* CONFIG_ELOOP_SELECT */
	return;
}
