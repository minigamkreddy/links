				     ...
				     const struct wpa_interface *iface)
{
	...
	printf("INitializing interface=%s, conf=%s, driver=%s, ctrl_interface=%s, bridge= %s\n", iface->ifname,
		   iface->confname ? iface->confname : "No" ,iface->driver ? iface->driver : "default",
		   iface->ctrl_interface ? iface->ctrl_interface : "NO",iface->bridge_ifname ? iface->bridge_ifname : "NO");
	...
	if (iface->ifname == NULL) {
		printf("iface->ifname =%s\n",iface->ifname);
		wpa_printf(MSG_ERROR, "\nInterface name is required.");
		return -1;
	}
	printf("wpa_s->ifname =%s\n",wpa_s->ifname);
	printf("os_strlen(iface->ifname = %d,sizeof(wpa_s->ifname = %d\n",os_strlen(iface->ifname),sizeof(wpa_s->ifname));
...
int wpa_supplicant_run(struct wpa_global *global)
{
	...
	if (global->params.daemonize &&
	    (wpa_supplicant_daemon(global->params.pid_file) ||
	     eloop_sock_requeue()))
		return -1;
//#define CONFIG_MATCH_IFACE 1
#ifdef CONFIG_MATCH_IFACE
//#ifdef 
	data = wpa_supplicant_match_existing(global);
	printf("data = %d\n",data);
	if (data)
		return -1;
#endif
